...
podTemplate(
    ...
    node(POD_LABEL) {
		...
		sh ' echo "FROM centos \n RUN yum update -y && yum install -y java && yum install -y tomcat  && cd / && cd usr/share/tomcat/conf && rm -rf server.xml && wget https://raw.githubusercontent.com/PeterBrave/CICDserver/master/server.xml && systemctl enable tomcat \n COPY /target/*.war /usr/share/tomcat/webapps/ \n ENTRYPOINT /usr/sbin/init" > Dockerfile'
		/*Build docker*/
